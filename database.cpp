...
public:
    Date (int new_year, int new_month, int new_day)
    {
        ...
        {
            throw out_of_range("Month value is invalid: " + std::to_string(new_month));
        } else if (new_day < 1 || new_day > 31)
        {
            throw out_of_range("Day value is invalid: " + std::to_string(new_day));
        }
        year = new_year;
        month = new_month;
        day = new_day;
...
public:
    ...
    set<std::string> Find(const Date& date) const
    {
        set<string> result;
        if (records.count(date) > 0)
        {
...
private:
    map<Date, set<std::string>> records;
};
